...
public class WordController {
    ...
    public Map<String, Object> wordstudyForm(@RequestParam String card) {
        ...
        //List<WordEntity> 형태로 WordService의 getWordByWordCode 메소드를 이용해 card에 해당하는 값 받아옴.
        ...
        if (!wordEntities.isEmpty()) { //리스트가 비어있는지 확인 - 비어있지 않을 때
            ...
            //response 맵에 count 키로 wordEntities.size() 넣음. - 슬라이드 동적 생성해야 해서 데이터 개수도 같이 보냄.
        } else { //리스트가 비어있을 때
    ...
    public Map<String, Object> wordstudyForm(@RequestParam String card) {
        ...
        if (!wordEntities.isEmpty()) {
            ...
            List<TextEntity> textEntity = textService.getTextByCard(card); // 이 부분은 실제 연관 관계에 따라 수정되어야 할 수 있음

            response.put("textTitle", textEntity != null ? textEntity.getTextTitle() : null);
